...
//Admin notice 
if( !function_exists('gboxnext_admin_notice')):
function gboxnext_admin_notice() {
    global $pagenow;
    ...
    $message = __( '<strong><span style="color:red;">Recommended WordPress Themes For You:</span>  <span style="color:green"> If you find a Secure, SEO friendly, full functional premium WordPress theme for your site then </span>  </strong>', 'niso' );

    printf( '<div class="%1$s" style="padding:10px 15px 20px;"><p>%2$s <a href="%3$s" target="_blank">'.__('see here','niso' ).'</a>.</p><a target="_blank" class="button button-danger" href="%3$s" style="margin-right:10px">'.__('View WordPress Themes','niso').'</a></div>', esc_attr( $class ), wp_kses_post( $message ),$url1 ); 
}
add_action( 'admin_notices', 'gboxnext_admin_notice' );
endif;
...
function wpspace_notice__error() {
	delete_option( 'nisonotice1' );
    if(get_option('nisonotice1')){
        return;
    }
	$class = 'niso-notice notice notice-warning is-dismissible';
	$message = __( '<strong><span style="color:red;">Hurry up:</span>  <span style="color:red"> <span style="color:green;">Upgrade Niso Carousel Pro For Lifetime With Nominal Price.</span> </span> And get Pro Update, unlimited features with Post Carousel, Product Carousel, Portfolio Carousel, Services Carousel, Team Carousel and regular image Carousel. </strong>', 'niso' );
