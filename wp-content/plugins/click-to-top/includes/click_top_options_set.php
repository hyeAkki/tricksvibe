...
a#clickTop {
  ...
  bottom: <?php echo $bottom_position; ?>%;
  color: <?php echo $icon_color; ?>;
  padding: <?php echo $scroll_padding; ?>px;
  <?php echo $scroll_position; ?>: <?php echo $selected_position; ?>%;
  <?php if($btn_type=='icon'): ?>
    min-height: 34px;
    min-width: 35px;
  <?php endif; ?>
  font-size:<?php echo $font_size ; ?>px;
  opacity:0.<?php echo $scroll_opacity; ?>
...
@-webkit-keyframes hvr-back-pulse {
  50% {
    background-color: <?php echo $bg_color; ?> none repeat scroll 0 0;
  }
}
...
.hvr-bubble-float-top::before {
  border-color: transparent transparent <?php echo $bg_color; ?>;
}
</style>
...
$btn_text = ( isset( $sstyle['btn_text'] ) ) ? $sstyle['btn_text'] : 'Scroll to top';
 ?>
...
(function ($) {
    ...
    $(document).ready(function(){
        ...
        scrollDistance: <?php echo $scroll_Distance; ?>, // Distance from top/bottom before showing element (px)
        scrollFrom: 'top',// 'top' or 'bottom'
        scrollSpeed: <?php echo $scroll_Speed; ?>, // Speed back to top (ms)
        easingType: '<?php echo $easing_Type ; ?>', // Scroll to top easing (see http://easings.net/)
        animation: '<?php echo $ani_mation; ?>', // Fade, slide, none
        animationSpeed: <?php echo $animation_Speed; ?>, // Animation speed (ms)
        scrollText: '<?php if($btn_type=='icon'): ?><i class=" fa fa-<?php echo $select_icon; ?>"></i><?php else: echo $btn_text; endif; ?>', // Text for element, can contain HTML
        activeOverlay: false,        // Set CSS color to display scrollUp active point, e.g '#00FFFF'
        zIndex: 2147483647           // Z-Index for the overlay
    });
		$('a#clickTop').addClass('hvr-<?php echo $hover_affect; ?>');
        });
}(jQuery));	
</script>
